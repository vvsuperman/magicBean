...
class WebsocketRequestImpl {
    ...
    Logger logger = LoggerFactory.getLogger(WebsocketRequestImpl.class);
    ...
    WebsocketRequest<List<MarkPriceEvent>> subscribeMarkPricesEvent( SubscriptionListener<List<MarkPriceEvent>> subscriptionListener,
        ...
        request.name = "mark prices";
        request.connectionHandler = (connection) -> connection.send(Channels.markPrices());
    ...
    WebsocketRequest<AggregateTradeEvent> subscribepremiumIndexEvent( SubscriptionListener<AggregateTradeEvent> subscriptionListener,
                                                                       SubscriptionErrorHandler errorHandler) {
        InputChecker.checker()
                .shouldNotNull(subscriptionListener, "listener");
        WebsocketRequest<AggregateTradeEvent> request = new WebsocketRequest<>(subscriptionListener, errorHandler);
        request.name = "***premiumIndex for";
        request.connectionHandler = (connection) -> connection.send(Channels.premiumIndex());
...
//                orderUpdate.setCommissionAmount(jsondata.getLong("n"));
            ...
            }else if (jsonWrapper.getString("e").equals("listenKeyExpired")){
                OrderUpdate orderUpdate = new OrderUpdate();
                JsonWrapper jsondata = jsonWrapper.getJsonObject("o");
                result.setEventType("LISTEN_KEY_EXPIRED");
            }
