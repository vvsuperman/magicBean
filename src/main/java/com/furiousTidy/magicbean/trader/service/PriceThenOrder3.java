...
public class PriceThenOrder3 {
    ...
    void getPriceInfoThenOrder2() throws InterruptedException {
        ...
        while(i<3){
            BigDecimal ethPrice1 = binanceClient.getFutureSyncClient().getMarkPrice("ETHUSDT").get(0).getMarkPrice();
            log.info("query price1:{}, timestamp={}",ethPrice1, LocalDateTime.now());
            Thread.sleep(500);

            BigDecimal ethPrice2 = binanceClient.getFutureSyncClient().getMarkPrice("ETHUSDT").get(0).getMarkPrice();
            log.info("query price2:{}, timestamp={}",ethPrice2,LocalDateTime.now());
